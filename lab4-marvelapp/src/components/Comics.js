...
const Comics = (props) => {
    ...
    const [ charsData, setComicData ] = useState(undefined);
    ...
    const buildCard = (comic) => {
        ...
        let url = `/comics/${comic.id}`;
        let alt = 'cover of '+comic.title;
		return (
    ...
    let card = "";
    if (charsData){
        card = charsData.map( (comic) => {return buildCard(comic)} );
    }
    ...
    if (!pagevalid){
		...
		return (
			<div>
                ...
                <br/>
				<PageNavButtons page={props.match.params.page} next={next} link='comics' />
				<br />
